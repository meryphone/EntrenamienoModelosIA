...
Para continuar, vamos a comenzar con el tratamiento de valorees
atípicos. Vamos a calcular el valor limite superior e inferior para
considerarlos atípicos. Vamos a repetir el cálculo con todas las
...
evitando la distorsión de diferencia de escalas como por el hecho de que
de esta manera se evitan problemas con los algoritmos de ajuste de
modelos que no posean la propiedad de invarianza al escalado.
...
Como podemos comprobar hemos aplicado la normalización correctamente, el
obtener valores negativos es completamente normal al aplicar este tipo
de normalización. Esto es debido a que la normalización robusta consiste
en centrar la variable en la mediana y escalar mediante el RIC. Por
tanto, los valores negativos significan que está el valor por debajo de
la mediana y los positivos que la supera.
...
tienen importancia en la adquisición de conocimiento del modelo. Por
tanto, concluimos con que no debemos realizar ningún tipo de reducción
de dimensionalidad a los datos.

Para continuar, vamos a proceder a la reducción del sesgo de las
distintas variables dado que ramdonForest se ve beneficiado mejorando la
...
lo que indica un buen rendimiento del modelo. La sensibilidad es
del 90.79%, lo que refleja que el modelo identifica correctamente la
mayoría de los casos positivos, y la especificidad es del 83.61%, lo que
muestra una buena capacidad para detectar negativos. El valor predictivo
positivo (PPV) del 87.34% y el valor predictivo negativo (NPV)
del 87.93% confirman que las predicciones son confiables en ambas
clases. El índice Kappa de 0.7476 sugiere un acuerdo sustancial entre
predicciones y datos reales, y el p-valor del test de McNemar (0.6276)
indica que no hay diferencias significativas entre las tasas de error de
las clases. Estos resultados validan que el modelo tiene un buen
equilibrio entre sensibilidad y especificidad, logrando una precisión
balanceada del 87.20%.
...
En el modelo `nnet` hemos ajustado dos hiperparámetros
clave: `size` y `decay`. El hiperparámetro `size` controla el número de
nodos en la capa oculta, lo que afecta la capacidad del modelo para
capturar patrones complejos; se probaron valores de 1, 3, 5 y 7,
representando desde configuraciones simples hasta arquitecturas más
complejas. El hiperparámetro `decay` es una tasa de regularización que
ayuda a evitar el sobreajuste al penalizar pesos grandes en la red; se
exploraron valores pequeños
como `1e-05`, `1e-04`, `1e-03` y `1e-02` para encontrar un equilibrio
...
El bucle evalúa distintas combinaciones de los
hiperparámetros `size` (nodos en la capa oculta)
y `decay` (regularización) en el modelo `nnet` para identificar la
configuración que maximiza la precisión en el conjunto de test. En cada
iteración, el modelo se entrena con una combinación específica de estos
valores sin validación cruzada (`method = "none"`) y se evalúa
directamente sobre el conjunto de test mediante una matriz de confusión
para calcular la precisión. Los resultados, incluyendo `size`, `decay` y
precisión, se almacenan y ordenan en un data frame para facilitar la
selección de los mejores hiperparámetros. Este enfoque permite controlar
directamente el rendimiento en datos no vistos y ajustar el equilibrio
entre la flexibilidad del modelo y su capacidad de generalización.
...
La gráfica muestra cómo varía la precisión del modelo **k-Nearest
Neighbors (k-NN)** en validación cruzada a medida que se ajusta el
número de vecinos (`#Neighbors`, representado por el hiperparámetro
`k`). Se observa que la mayor precisión (\~85.5%) se obtiene con k = 5,
lo que indica que considerar 5 vecinos para la clasificación balancea
bien la capacidad del modelo para generalizar. A medida que `k` aumenta,
la precisión disminuye de forma notable, alcanzando valores mínimos
alrededor de `k = 11` y `k = 19`, lo que sugiere que usar demasiados
vecinos puede diluir la influencia de los puntos más cercanos y
relevantes. Este comportamiento refleja que valores de `k` muy altos
pueden sobre-suavizar la clasificación, mientras que valores más bajos
permiten capturar mejor los patrones locales en los datos. Por lo tanto,
el modelo es más efectivo con valores pequeños de `k`, particularmente k
= 5 en este caso.
...
importancia_directa <- summary(gbm_model_original$finalModel)  # Si entrenaste con caret
...
vemos, esta transformación no ha mejorado el modelo, sino que se ha
...
```{r}
# Instalar si no tienes gridExtra
if(!require(gridExtra)) install.packages("gridExtra")

# Crear cada gráfico por separado
bw_plot <- bwplot(resultados, metric = "Kappa", main = "Boxplot - Kappa") # Boxplot
density_plot <- densityplot(resultados, metric = "Accuracy", main = "Density Plot - Accuracy") # Densidad Accuracy
density_plot_kappa <- densityplot(resultados, metric = "Kappa", main = "Density Plot - Kappa") # Densidad Kappa
dot_plot <- dotplot(resultados, metric = "Accuracy", main = "Dotplot - Accuracy") # Dotplot
...
estos dos modelos tienen un rendimiento confiable y generalizan mejor a
los datos. En resumen, los modelos GBM ajustado y RandomForest son los
más efectivos y consistentes en este análisis, mientras que los demás
modelos muestran un rendimiento aceptable pero menos competitivo.

##Elección del modelo final

Finalmente, basandonos en las pruebas realizadas nos decantamos por el modelo $GBM_{tuned}$ porque tiene el mejor rendimiento promedio en las gráficas y es más consistente. En el Dotplot, es el modelo con mayor precisión y un intervalo de confianza pequeño, lo que muestra que es confiable. En los Boxplots y Density Plots, también tiene valores altos y concentrados tanto en precisión como en Kappa, lo que significa que su desempeño es sólido en todas las pruebas. Aunque RandomForest también es un buen modelo, $GBM_{tuned}$ logra mejores resultados y es más estable, por eso sería la mejor opción.
