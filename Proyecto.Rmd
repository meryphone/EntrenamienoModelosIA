...
# Función para calcular límites de winsorización
calcular_limites <- function(x, lower_quantile = 0.05, upper_quantile = 0.95) {
  lower_limit <- quantile(x, lower_quantile, na.rm = TRUE)
...
winsorizar_test <- function(x, limites) {
  x[x < limites$lower_limit] <- limites$lower_limit
  x[x > limites$upper_limit] <- limites$upper_limit
  return(x)
}

# Variables a winsorizar
variables_winsorizar <- c("V8", "V11", "V15")

# Winsorización en conjunto de entrenamiento
limites_winsor <- list()
for (var in variables_winsorizar) {
  limites_winsor[[var]] <- calcular_limites(credit.Datos.Train.Normalizados[[var]])
  credit.Datos.Train.Normalizados[[var]] <- winsorizar_test(credit.Datos.Train.Normalizados[[var]], limites_winsor[[var]])
}
...
preproc <- preProcess(credit.Datos.Train.Normalizados[, c("V8", "V11", "V15")], method = "BoxCox")
...
```
```{r}
# Configurar gráficos en una ventana de 2 filas y 3 columnas
par(mfrow = c(2, 3))  

# Lista de columnas a graficar
columnas <- c("V8", "V11", "V15")

# Graficar cada variable antes de Box-Cox
for (col in columnas) {
  hist(
    credit.Datos.Train[[col]], 
    main = paste("Antes de Box-Cox:", col),
    xlab = col,
    col = "lightblue", 
    border = "white"
  )
...
# Graficar cada variable después de Box-Cox
for (col in columnas) {
  hist(
    credit.Datos.Train.Normalizados[[col]], 
    main = paste("Después de Box-Cox:", col),
    xlab = col,
    col = "lightgreen", 
    border = "white"
  )
...
# Winsorización en conjunto de prueba
for (var in variables_winsorizar) {
  credit.Datos.Test.Normalizados[[var]] <- winsorizar_test(credit.Datos.Test.Normalizados[[var]], limites_winsor[[var]])
}
...
control <- trainControl(
  ...
  number = 10,             # Aumentar particiones
)

# Rango de hiperparámetros ampliado
tuneGrid <- expand.grid(
  size = c(4, 6, 8, 10),           # Tamaño de capa oculta
  decay = c(0.01, 0.03, 0.05, 0.07) # Regularización
)

# Ajuste del modelo
modelo_nnet <- train(
  V16 ~ .,                        # Fórmula
  data = credit.Datos.Train.Normalizados,      # Datos de entrenamiento
  method = "nnet",                # Método nnet
  tuneGrid = tuneGrid,            # Rango de hiperparámetros
  trControl = control,            # Validación cruzada
  linout = FALSE,                 # Salida no lineal
  trace = FALSE,                  # No mostrar progreso interno
  maxit = 2000                     # Incrementar iteraciones
)
...
predicciones <- predict(modelo_nnet, newdata = credit.Datos.Test.Normalizados)

# Evaluación del rendimiento en los datos de prueba
library(caret)
conf_matrix <- confusionMatrix(predicciones, credit.Datos.Test.Normalizados$V16)
print(conf_matrix)
...
# Matriz de confusión
confusionMatrix(predicciones, credit.Datos.Test.Normalizados$V16)
...
El modelo GBM ha obtenido una precisión del 79.56%, lo que significa que aproximadamente el 80% de las predicciones coinciden con las etiquetas reales. La sensibilidad es del 86.84%, lo que indica que el modelo es bastante bueno detectando los casos positivos. La especificidad es del 70.49%, lo cual muestra que, aunque es efectivo identificando los negativos, tiene margen para mejorar. El valor predictivo positivo (78.57%) y el valor predictivo negativo (81.13%) indican que el modelo predice con razonable confianza tanto los positivos como los negativos. El índice kappa de 0.5809 sugiere un acuerdo moderado entre las predicciones del modelo y las etiquetas reales. En general, el modelo presenta un rendimiento sólido pero puede mejorarse, especialmente en la precisión de los negativos.
...
```{r}
# Actualización del control y grid
set.seed(1234)
control <- trainControl(
  method = "cv",              # Validación cruzada
  number = 10,                # Aumentamos a 10 folds
)

tuneGrid <- expand.grid(
  n.trees = c(500, 700, 1000),   # Mayor número de árboles
